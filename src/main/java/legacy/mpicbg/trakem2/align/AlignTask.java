...
import de.unihalle.informatik.rhizoTrak.display.VectorDataTransform;
	...
	*/
			...
			final Iterable< Tile< ? > > graph2 )
	{
		...
		for ( final Tile< ? > tile : graph1 )
			selection1.add( ( (legacy.mpicbg.trakem2.align.AbstractAffineTile2D< ? > )tile ).getPatch() );
		final Rectangle graph1Box = selection1.getBox();

		final Selection selection2 = new Selection( null );
		for ( final Tile< ? > tile : graph2 )
			selection2.add( ( (legacy.mpicbg.trakem2.align.AbstractAffineTile2D< ? > )tile ).getPatch() );
		...
		if ( features1.size() > 0 && features2.size() > 0 )
		{
			...
			if ( modelFound )
			{
				...
				for ( final Tile< ? > t : graph1 )
					( (legacy.mpicbg.trakem2.align.AbstractAffineTile2D< ? > )t ).initModel();
...
//			}
		...
		if ( largestGraphOnlyIn && ( hideDisconnectedTilesIn || deleteDisconnectedTilesIn ) )
		{
			...
			for ( final Tile< ? > t : largestGraph )
				tiles_to_keep.add( (legacy.mpicbg.trakem2.align.AbstractAffineTile2D< ? > )t );
